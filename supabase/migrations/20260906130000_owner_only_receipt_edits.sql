...
drop policy source_lots_open_update on public.source_lots;
create policy source_lots_owner_update
on public.source_lots for update to authenticated
...
begin
  if not public.is_owner() then raise exception 'Only the Owner can edit milk or cream receipts'; end if;
