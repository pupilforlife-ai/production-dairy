...
begin
  ...
  if target_batch.status = 'cancelled'::public.production_status then return target_batch.status; end if;
  if coalesce(target_batch.gross_output_quantity, 0) > 0 then
    raise exception 'This round has production output and cannot be cancelled from the board';
  end if;
  if exists (select 1 from public.production_round_packing_entries where production_batch_id = target_batch.id) then
    raise exception 'This round has packing entries and cannot be cancelled from the board';
  end if;
  if exists (select 1 from public.production_round_blocks where production_batch_id = target_batch.id and weight_kg is not null) then
    raise exception 'This round has recorded block weights and cannot be cancelled from the board';
  end if;
  if exists (select 1 from public.production_round_cream_buckets where production_batch_id = target_batch.id) then
    raise exception 'This round has cream buckets and cannot be cancelled from the board';
  end if;
  if exists (select 1 from public.transformations where source_batch_id = target_batch.id and status = 'completed'::public.transformation_status) then
    raise exception 'This round has completed transformations and cannot be cancelled from the board';
  end if;
  update public.production_batches
  set status = 'cancelled'::public.production_status, completed_at = null,
      notes = concat_ws(E'\n', nullif(notes, ''), 'Correction: cancelled accidental round — ' || cleaned_reason),
      updated_at = now(), updated_by = auth.uid()
  where id = target_batch.id;
  insert into public.audit_events (entity_type, entity_id, action, actor_user_id, old_data, new_data, reason)
  values ('production_batch', target_batch.id, 'cancel_accidental_round', auth.uid(), to_jsonb(target_batch), jsonb_build_object('status', 'cancelled'), cleaned_reason);
