...
create or replace function public.create_recipe_next_version(
...
begin
  ...
  where id = requested_recipe_id
    and active
  for update;
  if not found then raise exception 'Recipe is unavailable'; end if;
  ...
  where recipe_id = requested_recipe_id;

  update public.recipe_versions
  set effective_to = requested_effective_from - 1
  where recipe_id = requested_recipe_id
    and effective_to is null
    and effective_from < requested_effective_from;

  insert into public.recipe_versions (
    id, recipe_id, version_number, basis_quantity, basis_uom_id,
    effective_from, created_by
  ) values (
    generated_version_id, requested_recipe_id, next_version_number,
    requested_basis_quantity, requested_basis_uom_id,
    requested_effective_from, auth.uid()
  );
  ...
  loop
    ...
    if component_type = 'ingredient' then
      ...
      if component_material_id = target_recipe.product_id then
  ...
  return generated_version_id;
end;
$$;

revoke all on function public.create_recipe_next_version(
  uuid, numeric, uuid, date, jsonb
) from public, anon;
grant execute on function public.create_recipe_next_version(
  uuid, numeric, uuid, date, jsonb
) to authenticated;
